...
def enc_preprocess(img, target_image_size):
    s = min(img.size)
...
class PrepCoCoDS(Dataset):
    ...
    def __getitem__(self, item: int):
        ...
        try:
            image_d2 = enc_preprocess(PIL.Image.open(image_path).convert("RGB"), 128)
            image_d3 = enc_preprocess(PIL.Image.open(image_path).convert("RGB"), 256)
...
def main(bs, coco_dir, coco_ann_dir, out_dir):
    ...
    dvae_encoder = load_model("https://cdn.openai.com/dall-e/encoder.pkl", device)
    ...
    for suffix in ("val", "train"):
        ...
        for i, data in enumerate(dl):
            ...
            with torch.no_grad():
                z_logits2 = dvae_encoder(images_d2.squeeze(1))
                z2 = torch.argmax(z_logits2, axis=1).cpu()

                z_logits3 = dvae_encoder(images_d3.squeeze(1))
                z3 = torch.argmax(z_logits3, axis=1).cpu()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--out_dir', type=str, default="./prep_coco")
